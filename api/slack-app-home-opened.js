...
const publishLoadingPage = async user => {
  const result = await publishPage({ user, ...transcript('appHome.loading') })
  console.log(result)
  return result
}
...
const publishHomePage = async ({ user, results }) => {
  ...
  if (results.publicMeetings.length > 0) {
    blocks.push(
      transcript('appHome.publicMeetings', {
        publicMeetings: results.publicMeetings
      })
    )
    blocks.push(transcript('appHome.divider'))
  }
  if (results.recordings != {}) {
    if (results.recordings.processing.length > 0) {
      blocks.push(
        transcript('appHome.recordedMeetings.processing', {
          processingCount: results.recordings.processing.length
        })
      )
    }
    ...
    if (results.recordings.completed.length > 0) {
      ...
      blocks.push(
        transcript('appHome.recordedMeetings.completed', {
          completedRecordings
        })
      )
    }
  ...
  if (results.user) {
    ...
    if (sm.length > 1) {
      ...
      blocks.push(
        transcript('appHome.scheduledHostKeys.single', {
          hostKey: sm[0].meeting.fields['Host Key']
        })
      )
    } else {
...
const getRecordings = async user => {
  ...
  const completed = (
    await Promise.all(
      recordedMeetings
        .filter(record => {
          return record.fields['Recording Events'].includes(
            'recording.completed'
          )
