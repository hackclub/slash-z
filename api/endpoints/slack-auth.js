...
export default async (req, res) => {
  ...
  const user = await AirBridge.find('Authed Accounts', {
  ...
  if (user) {
    ...
    const slackData = await fetch(tokenUrl, { method: 'post' }).then(r =>
      r.json()
    )
    AirBridge.patch('Authed Accounts', recordID, {
      'Slack ID': slackData['authed_user']['id']
    })
