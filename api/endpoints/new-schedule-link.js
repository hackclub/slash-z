...
module.exports = async (req, res) => {
  ...
  if (!user.fields['Slack ID']) {
    ...
    const authUrl = `https://slack.com/oauth/v2/authorize?response_type=code&redirect_uri=${encodeURIComponent(redirectUrl)}&scope=users.identity&client_id=2210535565.1711449950551&state=${user.id}`
    return res.json({
      error: 'AUTH',
      authUrl
    })
