...
export default async (req, res) => {
  let user = await AirBridge.find('Authed Accounts', {
    filterByFormula: `{Name}='${req.query.id}'`
  })
  if (!user) {
  ...
  if (!user.fields['Slack ID']) {
    ...
    const authUrl = `https://js-slash-z.hackclub.com/auth-start.html?response_type=code&redirect_uri=${encodeURIComponent(
      redirectUrl
    )}&user_scope=identify&client_id=2210535565.1711449950551&state=${user.id}`
    return res.json({
      error: 'AUTH',
      authUrl
    })
