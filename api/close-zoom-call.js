...
export default async (zoomID, forceClose = false, fromWebhook = false) => {
  ...
  if (!forceClose && metrics && metrics.total_records > 0) {
    console.log(
      `Meeting ${meeting.zoomID} has ${metrics?.total_records.length || "unknown"} participant(s). Not closing meeting. Run with forceClose=true to force close the meeting even with participants.`
    );
    return null;
  }
  ...
  if(!fromWebhook){
    ...
    await Prisma.create('customLogs', { text: `slash_z_ended_call_${metrics?.total_records.length || "unknown"
